...
class DBUtility:
    def getDBConfiguration():
        if path.isfile("dbconfig.xml")==False:
            raise DataLayerException("dbconfig.xml with database connection detail is missing so refer documentation")
        ...
        if port!=None:
            try:
                ...
                raiseDataLayerException(f"Port in dbconfig.xml is of type {type(port)} ,it should be of type {type(10)} ")
...
class DBConnection:
    def getConnection():
        ...
        try:
            connection=connect(host=dbConfiguration.host,port=dbConfiguration.port,database=dbConfiguration.database,username=dbConfiguration.user,password=dbConfiguration.password)        
        except Error as error:
            raise DataLayerException(message=error.msg)
        return connection
...
class HRDLHandler:
    def add_designation(designation):
        ...
        if isinstance(designation,Designation)==False: raise DataLayerException(f"Type class<Designation> required,found {type(designation)}")
        if designation.has_exceptions: raise DataLayerExceptions(exceptions=designation.exceptions)
        if designation.code!=0: raise DataLayerException("Designation Code should be 0 as it will be auto generated")
        try:
    ...
    def update_designation(designation):
        ...
        if isinstance(designation,Designation)==False: raise DataLayerException(f"Type class<Designation> required,found {type(designation)}")
        if designation.has_exceptions: raise DataLayerExceptions(exceptions=designation.exceptions)
        if designation.code==0: raise DataLayerException("Code required it can not be 0")
        try:
            dbConnection=DBConnection.getConnection()
            cursor=dbConnection.cursor()
            cursor.execute("select code from designation where code=%s",(designation.code,))
            rows=cursor.fetchall()
            if len(rows)==0: raise DataLayerException(f" Code : {designation.code} does not exists")
    ...
    def delete_designation(code):
        if code<=0: raise DataLayerExeption(f"Invalid code : {code}")
        try:
            dbConnection=DBConnection.getConnection()
            cursor=dbConnection.cursor()
            cursor.execute("select code from designation where code=%s",(code,))
            rows=cursor.fetchall()
            if len(rows)==0: raise DataLayerException(f" Code : {code} does not exists")
    ...
    def get_designation_by_code(code):
        if code==None: raise DataLayerException("Code required")
        if isinstance(code,int)==False: raise DataLayerException(f"Type {type(10)} required,found {type(code)}")
        ...
        try:
            ...
            row=cursor.fetchone()
            if row==None: raise DataLayerException(f"Invalid code : {code}")
            designation=Designation(code,row[1])
            dbConnection.commit()
        except Error as error:
            raise DataLayerException(error.msg)
        finally: 
    ...
    def get_designation_by_title(title):
        if title==None: raise DataLayerException("Title required")
        if isinstance(title,str)==False: raise DataLayerException(f"Type {type('A')} required,found {type(title)}")
        if len(title)==0 or len(title)>35: raise DataLayerException(f"Invalid title : {title}")
        try:
            dbConnection=DBConnection.getConnection()
            cursor=dbConnection.cursor()
            cursor.execute("select * from designation where title=%s",(title,))
            row=cursor.fetchone()
            if row==None: raise DataLayerException(f"Invalid title : {title}")
            designation=Designation(row[0],title)
            dbConnection.commit()
        except Error as error:
            raise DataLayerException(error.msg)
        finally: 
